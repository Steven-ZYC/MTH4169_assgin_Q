...
\begin{document}
	...
	We systematically analyze how parameters (chambers, bullets, players) affect death probabilities. Rather than making decisions, we calculate the exact probabilities in multiple scenarios to discover the general patterns and derive a fairness theorem. We also define fairness as equal death probability for all players regardless of position. 
	...
	\begin{adjustwidth}{2em}{0pt}
		...
		\end{itemize}
		\begin{align*}
			\mathbb{P}(\text{Player 2 dies})=\mathbb{P}(\text{Player 1 survives}) \times \mathbb{P}(\text{Player 2 hit bullets} \mid \text{Player 1 survived})=\frac{C-n}{C} \times \frac{n}{C-1}
		\end{align*}
		\textbf{For Player 3:}
		...
		\end{itemize}
		\begin{align*}
			\mathbb{P}(\text{Player 3 dies})=\mathbb{P}(\text{Both P1 and P2 survive}) \times\mathbb{P}(\text{P3 hits bullet}\mid\text{both survived}) = \frac{C-n}{C}\times\frac{C-n-1}{C-1}\times\frac{n}{C-2} 
		\end{align*}
		...
		\begin{align*}
			...
			&= \frac{C-n}{C}\times\frac{C-1-n}{C-1} \times\frac{C-2-n}{C-2}\times\dots\times\frac{C-i+2-n}{C-i+2}\times\frac{n}{C-i+1}\\
		\end{align*}
		Where: 
		\begin{itemize}
		...
		\begin{align*}
			\mathbb{P}(\text{Player \textit{i} dies})
			&= \frac{C-1}{C}\times\frac{C-2}{C-1}\times\frac{C-3}{C-2}\times\dots\times\frac{C-\textit{i}+1}{C-\textit{i}+2}\times\frac{1}{C-\textit{i}+1}
		\end{align*}
		
		This is a \textbf{telescoping product}. Let's write out the full equation:
		\begin{align*}
		...
		We can now calculate probabilities for configurations we have not analyzed before.\\[4pt]
	...
	\begin{adjustwidth}{2em}{0pt}
		...
		\begin{align*}
			\text{Fair} \Longleftrightarrow \mathbb{P}(\text{Player 1 dies}) = \mathbb{P}(\text{Player 2 dies}) = ... = \mathbb{P}(\text{Player m dies})
		\end{align*}
		...
		Based on our systematic explorations above, we can now state our main throretical result. \\[4pt]
		\textbf{Theorem (Fairness Condition)}
		
		For a Russioan Roulette game with C chambers, n bullets, and m players (where $m \le C$ and $n \le C$), the game is fair if and only if $n=1$ \\
		...
		\begin{align*}
			...
			&= \frac{C-1}{C} \times \frac{C-2}{C-1} \times \frac{C-3}{C-2} \times\dots\times\frac{C-(i-1)}{C-(i-2)} \times \frac{1}{C-(i-1)} \\[6pt]
			&= \frac{1}{C}
		\end{align*}
		...
		This shows that \textbf{if} Player 1 and Player 2 have equal death probabiilty, \textbf{then} n must equal 1. 
		...
		However, Player 2 only reaches their turn if Player 1 survives, which happens with probability $\frac{C-n}{C}$. The combined effect still favors Player 2 when $n \geq 2$.\\[4pt]
	...
	\begin{adjustwidth}{2em}{0pt}
		In this part, we will be analyzing the optimal stopping problem, to find out when will be the best point to stop when we play in Russian Roulette that involve values for win/forfeit/lose. On the other word, \textbf{WHEN should you quit to avoid lose?} To find this out in a more interesting way, we implement a background for the game as follows: \textbf{\textit{The Prisoner's Tournament}}. Two prisoners are offered a chance at freedom through a Russian Roulette tournament with the following rules:\\[4pt]
		\textbf{Setup}
		\begin{itemize}
	...
	\begin{adjustwidth}{2em}{0pt}
		...
		Let $V_k$ = Expected Utility of continuing from Round k onward (before taking your shot in Round k)
		\begin{align*}
			V_k=\mathbb{P}(die)\times U_{death}+\mathbb{P}(win)_k\times U_{win}+\mathbb{P}(\textit{both survive})_k\times V_{k+1}
			=\frac{n}{C-2k+2}\times(-1)+\frac{C-2k+2-n}{C-2k+2}\times\frac{n}{C-2k+1}\times(10)+\frac{C-2k+2-n}{C-2k+2}\times{C-2k+1-n}{C-2k+1}\times V_{k+1}
		\end{align*}
...
\begin{document}
	...
	\begin{adjustwidth}{2em}{0pt}
		...
		The general formula is:
		\begin{align}
			P(\text{Player \textit{i} dies})=\lbrack\prod_{j=1}^{i-1} \frac{C-j+1-n}{C-j+1}\rbrack \times \frac{n}{C-i+1}
		\end{align}
		Expanded form:
		\begin{align*}
			\mathbb{P}(\text{Player \textit{i} dies})
	...
	\begin{adjustwidth}{2em}{0pt}
		...
		\begin{align*}
			\text{Fair} \Longleftrightarrow P(\text{Player 1 dies}) = P(\text{Player 2 dies}) = ... = P(\text{Player m dies})
		\end{align*}
		\textbf{Equation To Be Implemented} 
		...
		To quantify how unfair a game is, we can define a \textbf{fairness} index as the standard deviation of death probabilities across all players. For two players:
		\begin{align*}
			\text{Fairness Index}=\sqrt{\frac{{\lbrack P(P_1)-\bar{P}\rbrack}^{2}+{\lbrack P(P_2)-\bar{P}\rbrack}^{2}}{2}}
		\end{align*}
		where $\bar{P}=\frac{P(P_1)+P(P_2)}{2}$ is the mean death probability. \\[4pt]
	...
	\begin{adjustwidth}{2em}{0pt}
		...
		If we continue round K, three possible outcomes may occur: 
		\begin{enumerate}
			\item Probability of you die in round K:\\$P(\text{you die in round k})=\frac{n}{C-2k+2}$\\
			\item Probability of you surviving but opponent dies in:\\$P(\text{you win in round k}=\frac{C-2k+2-n}{C-2k+2})\times\frac{n}{C-2k+1}$\\
			\item Both survived (game continues):\\$P(\text{both survive})=\frac{C-2k+2-n}{C-2k+2}\times\frac{C-2k+1-n}{C-2k+1}$\\
		\end{enumerate}
